...
```{r}
setwd("C:/Users/Jim/Desktop/jen/Data 101/Week 4") 
...
```
```{r}
diversity_new$name[grepl('Mildred Elley', diversity_new$name)] <- 'Mildred_Elley'
```

### Name change for the entire group containing a portion of the name
```{r}
diversity_new$name[grepl('West Coast University', diversity_new$name)] <- 'West_Coast_University'
```

### Verifying that the group name change worked for different groups
```{r}
diversity_new$name[36875]
diversity_new$name[49766]
```
